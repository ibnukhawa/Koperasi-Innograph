...
class SisaHasilUsaha(models.Model):
    _name = 'ksp.shu'

    name = fields.Char(string='Sisa Hasil Usaha',  copy=False,  index=True, default=lambda self: _('New'))
    partner_id = fields.Many2one('res.partner','Nasabah')
    shu_ids = fields.One2many('ksp.shu.detail', 'shu_id', string='Sisa Hasil Usaha')
    ...
    def _compute_total(self):
        for doc in self:
            amount_total = sum(doc.shu_ids.mapped('total'))
            doc.amount_total = amount_total
    
    @api.depends('shu_ids')
    def _compute_wajib(self):
        for doc in self:
            amount_total = sum(doc.shu_ids.mapped('simpanan_wajib'))
            doc.wajib_id = amount_total

    @api.depends('shu_ids')
    def _compute_sukarela(self):
        for doc in self:
            amount_total = sum(doc.shu_ids.mapped('simpanan_sukarela'))
            doc.sukarela_id = amount_total

    @api.depends('shu_ids')
    def _compute_pokok(self):
        for doc in self:
            amount_total = sum(doc.shu_ids.mapped('simpanan_pokok'))
            doc.pokok_id = amount_total
    

class simpananDetail(models.Model):
    _name = 'ksp.shu.detail'
    shu_id = fields.Many2one('ksp.shu', string='Simpanan')
